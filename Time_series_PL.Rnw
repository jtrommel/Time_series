...
\section{Poging tot eigen decompositie met lowess()}

\subsection{Trend met lowess()}
<<>>=
JT.train.trend <- lowess(data_train$value, f=0.33) # Gives smoothed values with default settings - 1008 values
JT.rest <- data_train$value - JT.train.trend$y
JT.decomp <- data.table(Load = c(data_train$value, JT.train.trend$y, JT.rest),
                         Date = rep(data_train[,date_time], 3),
                         Type = factor(rep(c("original data", "lowess data", "rest"), 
                                           each = nrow(data_train)),
                                       levels = c("original data", "lowess data", "rest")))
@
...
ggplot(data = JT.decomp, aes(x = Date, y = Load)) + 
  geom_line() +
  facet_grid(Type ~ ., scales = "free_y", switch = "y") +
  labs(x="Date", y=NULL, title="Trend met lowess()" ) + 
  theme_ts
@
...
JT.decomp <- data.table(Load = c(data_train$value, JT.train.trend$y, JT.rest, JT.season$value, JT.ruis),
                         Date = rep(data_train[,date_time], 5),
                         Type = factor(rep(c("original data", "lowess data", "rest", "season", "ruis"), 
                                           each = nrow(data_train)),
                                       levels = c("original data", "lowess data", "rest", "season", "ruis")))
@
