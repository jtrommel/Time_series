...
In our data we have values sampled every half hour for each day. The periode of the original signal $T_{0}$ equals 1 day and the sample periode $T_{s}$ equals $\frac{1}{48}=\frac{T_{0}}{48}$ of a day. The number of samples N equals 48. The Discrete Fourier Transform gives the complex spectrum as:
