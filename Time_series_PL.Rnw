...
Calculating average per week (using dplyr):

<<>>=
# week function starts at 1 jan. In 2016 that is a Friday. We want our week to start on a monday so we do +4
data_train$JTweek <- week(data_train$date + 4) 
data_train %>% group_by(JTweek) %>% summarise(mean(value))
@

Calculate weekly averages:

<<>>=
data_train %>% group_by(JTweek) %>% summarise(mean(value)) -> averages
colnames(averages) <- c("JTweek","wkmean")
data_train <- left_join(data_train,averages)
ggplot(data_train) + 
  geom_line(aes(x=date_time, y=value)) + 
  geom_line(aes(x=date_time,y=wkmean),linetype=5, alpha=0.75, size=1.2, color="firebrick2") + 
