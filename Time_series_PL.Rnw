...
After decomposition the values of the trend are stored in decomp_ts in the second column. We use the ARIMA function for spotting the trend and then use this to make a forecast for the next period (the data_test period).
