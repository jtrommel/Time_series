...
\textsf{R} has a number of inbuilt function to find a trend. Best known are the \testtt{loess}- and \texttt{lowess}-functions. \sidenote{There is an excellent YouTube video on the inner workings of the \texttt{loess}- and \texttt{lowess}-function: https://www.youtube.com/watch?v=85XU1T9DIps}

<<label=JTloessmod,fig=TRUE,include=FALSE, echo=FALSE>>=
loess.model <- loess(JT.example1.df$value ~ JT.example1.df$nr, span=0.5)
...
ggplot(data=JT.example1.df) + 
  ...
  scale_y_continuous(breaks=seq(0,10,1)) +
  labs(title="Trend by Loess-functie (span=0.5)") +
  JT.theme
@
