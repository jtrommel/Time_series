...
The Periodic component can add a lot of clutter to the data and influence the accuracy of the functions (lm, loess) that we use to find the Trend. However: looking for the existence of periodic elements with e.g. a Discrete Fourier Transform is hindered by the presence of a trend.

<<echo=FALSE>>=
signal %>% select(t,y32) -> signal.y32
period.info <- JTR.FFT(signal.y32)
@

<<label=FFTy32,fig=TRUE,include=FALSE, echo=FALSE>>=
...
\begin{marginfigure}[-7cm]
\includegraphics[width=1\textwidth]{TS-FFTy32}
\label{fig:FFTy32}
\setfloatalignment{b}% forces caption to be bottom-aligned
\end{marginfigure}

We can see the Level clearly in this graph: it's the value at frequency=0. Because the time-step is equal to 1, the sampling period $T_{s}$ is also equal to 1 and the sampling frequency $F_{s}=\frac{1}{T_{s}}=1$. The smaller frequencies that the FFT can find lie in the interval $ [0,\frac{F_{s}}{2}]=[0,0.5]$ and the frequency resolution is $\frac{F_{s}}{N}=\frac{1}{101} \approx 0.01Hz$.
