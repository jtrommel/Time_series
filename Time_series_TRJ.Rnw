...
periodic <- 2*sin(2*pi*t/20) + sin(2*pi*t/25 + pi/6)
...
\newthought{To investigate different methods for finding the trend} we will use as an example a data series constructed with a Level, a (polynomial) Trend, periodicity (sum of two sines) and noise. This is the series y32 at the bottom right of Figure~\ref{fig:JTvisual}.
...
<<>>=
lin.model <- lm(signal$y32 ~ signal$t)
summary(lin.model)
...
ggplot(data=signal) + 
  geom_point(aes(x=t, y=y32), size=1, color="red") +
  geom_linerange(aes(x=t, ymin=0,ymax=y32),linetype=3, size=0.5) +
  geom_line(aes(x=t, y=lin.mod.y32), color="blue") +
  scale_y_continuous(breaks=seq(-5,15,1)) +
  labs(title="Trend of y32 by linear model") +
  JT.theme
@
...
<<>>=
poly3.model <- lm(signal$y32 ~ signal$t + I(signal$t^2) + I(signal$t^3))
summary(poly3.model)
...
ggplot(data=signal) + 
  ...
  scale_y_continuous(breaks=seq(-5,15,1)) +
  labs(title="Trend of y32 by polynomial model") +
  JT.theme
@
...
<<echo=FALSE>>=
poly2.model <- lm(signal$y32 ~ signal$t + I(signal$t^2))
poly4.model <- lm(signal$y32 ~ signal$t + I(signal$t^2) + I(signal$t^3) + I(signal$t^4))
@
