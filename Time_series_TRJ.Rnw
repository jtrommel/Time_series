...
\newthought{Let's do a rerun of the procedure}. We worked as follows:

\begin{enumerate}
  \item We start with the original signal y32
  \item We calculate the Level
  \item Substracting the Level from y32 we obtain y32L. This signal is not usable for the Discrete Fourier Transform because it contains a trend. Therefore we cannot determine the Periodic component at this moment
  \item We make a first estimate of the Trend using a polynomial regression or by applying a \texttt{loess}-function on y32L. Both the polynomial regression and the \texttt{loess}-approximation are hindered by the presence of the Periodic component (which we do not know at the moment) and the Noise
  \item We eleminate the Trend: y32LT=y32L - Trend
  \item We find the Periodic component using DFT or by calculating the mean of the different periods. Important: the \emph{period} we use is the Least Common Multiple of the periods we find with the Discrete Fourier Transform
  \item The remainder is Noise
  \item The original signal can be deconstructed into Level+Trend, Periodic and Noise
\end{enumerate}

\newthought{After the first run} we now have a first approximation of the Periodic component. So we can redo the analysis but in a different sequence:

\begin{enumerate}
  \item We start with the original signal y32
  \item We calculate the Level
  \item Substracting the Level from y32 we obtain y32L. 
  \item We substract from y32L the Periodic component we found in the first run. This gives us y32LP
  \item We make a new estimate of the Trend using a polynomial regression or by applying a \texttt{loess}-function on y32LP. Let's call this one TrendNew
  \item The remainder is Noise
  \item The original signal can be deconstructed into Level+TrendNew, Periodic and Noise.
\end{enumerate}

<<label=deconloessBIS,fig=TRUE,include=FALSE, echo=FALSE>>=
signal$Periodic <- rep(period.gem.loess$gem[1:50],2)
signal$y32LP <- signal$y32L - signal$Periodic
loess.model <- loess(signal$y32LP ~ signal$t, span=0.9, degree=1)
signal$loess.y32LP <- predict(loess.model)
signal %>% select(t, y32, loess.y32LP) -> decon.loess
decon.loess$LevelplusTrend <- mean(decon.loess$y32) + decon.loess$loess.y32LP
decon.loess$Periodic <- signal$Periodic
decon.loess$Noise <- decon.loess$y32 - decon.loess$LevelplusTrend - decon.loess$Periodic
rms.noise.loess <- sqrt(sum(decon.loess$Noise^2))
decon.L <- data.frame(t = rep(decon.loess$t, 4) ,
                      value = c(decon.loess$y32, 
                                decon.loess$LevelplusTrend, 
                                decon.loess$Periodic, 
                                decon.loess$Noise),
                      type = factor(rep(c("original",
                                        "LevelplusTrend", 
                                        "Periodic",
                                        "Noise"), each=nrow(decon.loess)), 
                                  levels=c("original","LevelplusTrend", "Periodic","Noise")))
ggplot(data=decon.L, aes(x=t, y=value)) +
  geom_line() +
  facet_grid(type ~ ., scales = "free_y", switch = "y") +
...
\includegraphics[width=1\textwidth]{TS-deconloessBIS}
\caption{}
\label{fig:deconloessBIS}
\setfloatalignment{b}
\end{marginfigure}

Figure~\ref{fig:deconloessBIS} gives the result. From the calculation of the RMS value of the error we see that a second run does not lead to an improvement!
<<>>=
rms.error.loess
rms.noise.loess
@
...
\section{Using the deconstruction method on a real time series}

\newthought{In her book} ''Practival Time Series Forecasting with R", Galit Shmueli works with a time series called ''Amtrak data". These include the monthly ridership on Amtrak trains (in thousands) for Jan-1991 to March-2004.
...
\begin{figure}
\includegraphics[width=1\textwidth]{TS-Amdata}
\caption{}
\label{fig:Amdata}
\setfloatalignment{b}
\end{figure}

In Figure~\ref{fig:Amdata} we can see that a first degree linear trend is probably not the right thing to use. We could go for a polynomial trend, but the use of the \texttt{loess}-function of degree 2 seems more appropriate.

\subsection{Level and Trend}

<<label=AmLandT,fig=TRUE,include=FALSE, echo=FALSE>>=
Level <- mean(Amtrak.data$Ridership)
Amtrak.data$RidershipL <- Amtrak.data$Ridership - Level
loess.model <- loess(Amtrak.data$RidershipL ~ Amtrak.data$nr, span=0.9, degree=2)
Amtrak.data$Trend <- predict(loess.model)
Amtrak.data$LevelandTrend <- Amtrak.data$Trend + Level
 ggplot(data=Amtrak.data) + 
  geom_point(aes(x=nr, y=Ridership), size=1, color="red") +
  geom_line(aes(x=nr, y=LevelandTrend), color="blue") +
  labs(title="Level and Trend of Ridership using a linear model") +
  JT.theme
@

\begin{marginfigure}[-8cm]
\includegraphics[width=1\textwidth]{TS-AmLandT}
\caption{}
\label{fig:AmLandT}
\setfloatalignment{b}
\end{marginfigure}
...
First we use the Discrete Fourier Transform to get an idea of the periodicity (Figure~\ref{fig:AmFFT}).

<<label=AmFFT,fig=TRUE,include=FALSE, echo=FALSE>>=
Amtrak.data$RidershipLT <- Amtrak.data$Ridership - Amtrak.data$LevelandTrend 
Amtrak.data %>% select(nr, RidershipLT) -> signal.test
period.info <- JTR.FFT(signal.test)
freqspec <- period.info$freqspec
ggplot(data=freqspec) + 
  geom_line(aes(x=freq,y=amp)) +
  labs(title="Frequency spectrum of Ridership \n without Level and Trend (loess)") +
  JT.theme
@

\begin{marginfigure}[-2cm]
\includegraphics[width=1\textwidth]{TS-AmFFT}
\caption{}
\label{fig:AmFFT}
\setfloatalignment{b}
\end{marginfigure}

Three clear peaks at 0.082, 0.252. The corresponding periods are:

<<>>=
period.info$resultaat$period <- 1/period.info$resultaat$freq
period.info$resultaat$period
@

These periods are equal to 1 year and 1 quarter. \emph{the Least Common Multiple} period is the year, or T.sig=12.

<<label=AmPeriodic,fig=TRUE,include=FALSE, echo=FALSE>>=
Amtrak.data %>% select(nr,RidershipLT) -> period.form
T.sig <- 12
# Select an integer multiple of T.sig
Int.mult <- floor(nrow(Amtrak.data)/T.sig)
N <- T.sig*Int.mult
period.form <- period.form[1:N,]
period.form$period.nr <- rep(c(1:Int.mult) , each = T.sig)
period.form$period.nr <- as.factor(period.form$period.nr)
period.form$time <- rep(c(1:T.sig) , 13)
period.form %>% select(time, RidershipLT) %>% group_by(time) %>% summarize(mean(RidershipLT)) -> period.gem.loess
names(period.gem.loess) <- c("time","gem")
ggplot(data=period.form) +
  geom_line(aes(x=time, y=RidershipLT, color=period.form$period.nr)) +
  labs(title="Periodic signal for different periods") +
  scale_colour_discrete(name = "period.nr") +
  geom_line(data=period.gem.loess, aes(x=time, y=gem), size=1.5, color="black") +
  JT.theme
@
...
\begin{marginfigure}[0cm]
\includegraphics[width=1\textwidth]{TS-AmPeriodic}
\caption{}
\label{fig:AmPeriodic}
\setfloatalignment{b}
\end{marginfigure}

Deconstructing the Amtrak data:

<<label=deconAmtrak,fig=TRUE,include=FALSE, echo=FALSE>>=
Amtrak.data$Periodic <- c(rep(period.gem.loess$gem, Int.mult), period.gem.loess$gem[1:(nrow(Amtrak.data)-N)])
Amtrak.data %>% select(nr, Ridership, LevelandTrend, Periodic) -> decon.Amtrak
decon.Amtrak$Noise <- decon.Amtrak$Ridership - decon.Amtrak$LevelandTrend - decon.Amtrak$Periodic
rms.noise.Amtrak <- sqrt(sum(decon.Amtrak$Noise^2))
decon.L <- data.frame(t = rep(decon.Amtrak$nr, 4) ,
                      value = c(decon.Amtrak$Ridership, 
                                decon.Amtrak$LevelandTrend, 
                                decon.Amtrak$Periodic, 
                                decon.Amtrak$Noise),
                      type = factor(rep(c("original",
                                        "LevelandTrend", 
                                        "Periodic",
                                        "Noise"), each=nrow(decon.Amtrak)), 
                                  levels=c("original","LevelandTrend", "Periodic","Noise")))
ggplot(data=decon.L, aes(x=t, y=value)) +
  geom_line() +
  facet_grid(type ~ ., scales = "free_y", switch = "y") +
  labs(title="Deconstructed Amtrak with loess Trend") +
  JT.theme
@

\begin{figure}
\includegraphics[width=1\textwidth]{TS-deconAmtrak}
\caption{}
\label{fig:deconAmtrak}
\setfloatalignment{b}
\end{figure}
